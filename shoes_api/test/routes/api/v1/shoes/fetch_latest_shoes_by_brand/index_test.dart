...
import '../../../../../../routes/api/v1/shoes/fetch_latest_shoes_by_brand/index.dart'
    as route;
...
void main() {
  ...
  group('GET /', () {
    ...
    test(
        'should HttpStatus.notFound and an error message when fetchLatestShoesByBrand returns a Left',
        () async {
      //arrange
      when(() => mockRequest.method).thenReturn(HttpMethod.get);
      when(() => mockRequest.uri).thenReturn(
    ...
    test(
        'should HttpStatus.ok and a Json list of shoes when fetchLatestShoesByBrand returns a Right',
        () async {
      //arrange
      when(() => mockRequest.method).thenReturn(HttpMethod.get);
      when(() => mockRequest.uri).thenReturn(
      ...
      when(() => mockShoesRepository.fetchLatestShoesByBrand(
          brand: any(named: 'brand'),
        ),)
          .thenAnswer(
      ...
      verify(() => mockRequestContext.request).called(2);
      verify(() => mockRequestContext.read<ShoesRepository>()).called(1);
      verify(() => mockShoesRepository.fetchLatestShoesByBrand(
          brand: any(named: 'brand'),
        ),)
          .called(1);
