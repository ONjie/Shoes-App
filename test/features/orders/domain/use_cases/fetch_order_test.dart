...
void main() {
  ...
  test('should return Right(OrderEntity) from OrdersRepository', () async {
    //arrange
    when(
      () => mockOrdersRepository.fetchOrder(orderId: tOrder.orderId),
    ).thenAnswer((_) async => Right(tOrder));

    //act
    final result = await fetchOrder.call(orderId: tOrder.orderId);
