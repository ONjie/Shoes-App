...
void main() {
  ...
  late FetchOtherShoesByBrand fetchOtherShoesByBrand;
  late MockShoesRepository mockShoesRepository;

  setUp(() {
    mockShoesRepository = MockShoesRepository();

    fetchOtherShoesByBrand = FetchOtherShoesByBrand(shoesRepository: mockShoesRepository);
  });
  ...
  test('should return Right(List<ShoeEntity>) from ShoesRepository', () async {
    ...
    //arrange
    when(() => mockShoesRepository.fetchOtherShoesByBrand(brand: any(named: 'brand')))
        .thenAnswer((_) async => Right(tShoeEntityList));

    //act
    final result = await fetchOtherShoesByBrand.call(brand: tBrand);

    //assert
    expect(result, Right(tShoeEntityList));
    verify(() => mockShoesRepository.fetchOtherShoesByBrand(brand: any(named: 'brand'))).called(1);
