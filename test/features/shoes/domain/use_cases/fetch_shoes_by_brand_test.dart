...
void main() {
  ...
  late FetchShoesByBrand fetchShoesByBrand;
  late MockShoesRepository mockShoesRepository;

  setUp(() {
    mockShoesRepository = MockShoesRepository();

    fetchShoesByBrand = FetchShoesByBrand(shoesRepository: mockShoesRepository);
  });
  ...
  test('should return Right(List<ShoeEntity>) from ShoesRepository', () async {
    //arrange
    when(() => mockShoesRepository.fetchShoesByBrand(brand: any(named:'brand')))
        .thenAnswer((_) async => Right(tShoeEntityList));

    //act
    final result = await fetchShoesByBrand.call(brand: tBrand);

    //assert
    expect(result, Right(tShoeEntityList));
    verify(() => mockShoesRepository.fetchShoesByBrand(brand: any(named: 'brand'))).called(1);
