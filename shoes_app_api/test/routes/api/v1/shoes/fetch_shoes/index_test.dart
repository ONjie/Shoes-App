...
import '../../../../../../routes/api/v1/shoes/fetch_shoes/index.dart' as route;
...
void main() {
  ...
  group('GET /', () {
    ...
    test(
        'should HttpStatus.notFound and an error message when fetchShoes returns a Left',
        () async {
      //arrange
      when(() => mockRequest.method).thenReturn(HttpMethod.get);
      when(() => mockShoesRepository.fetchShoes()).thenAnswer(
        (_) async => const Left(
      ...
      verify(() => mockShoesRepository.fetchShoes()).called(1);
     ...
     test(
        'should HttpStatus.ok and a Json list of shoes when fetchShoes returns a Right',
        () async {
      //arrange
      when(() => mockRequest.method).thenReturn(HttpMethod.get);
      when(() => mockShoesRepository.fetchShoes()).thenAnswer((_) async =>
         const Right([tShoe]),);
