...
import '../../../../../../routes/api/v1/shoes/fetch_latest_shoes/index.dart'
    as route;
...
void main() {
  ...
  group('GET /', () {
    ...
    test(
        'should HttpStatus.notFound and an error message when fetchLatestShoes returns a Left',
        () async {
      //arrange
      when(() => mockRequest.method).thenReturn(HttpMethod.get);
      when(
        () => mockShoesRepository.fetchLatestShoes(),
      ).thenAnswer(
        (_) async => const Left(
    ...
    test(
        'should HttpStatus.ok and a Json list of shoes when fetchLatestShoes returns a Right',
        () async {
      //arrange
      when(() => mockRequest.method).thenReturn(HttpMethod.get);
      when(
      ...
      verify(() => mockRequestContext.request).called(1);
      verify(() => mockRequestContext.read<ShoesRepository>()).called(1);
      verify(() => mockShoesRepository.fetchLatestShoes()).called(1);
